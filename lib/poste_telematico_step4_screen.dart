// lib/screens/field_worker/poste_telematico_step4_screen.dart
...
class PosteTelematicoStep4Screen extends StatefulWidget {
  ...
  final String estado;
  final String inclinacion;
  final String altura;
  final String propietario;

  const PosteTelematicoStep4Screen({
    Key? key,
  ...
  @override
  _PosteTelematicoStep4ScreenState createState() =>
      _PosteTelematicoStep4ScreenState();
}

class _PosteTelematicoStep4ScreenState
    extends State<PosteTelematicoStep4Screen> {
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'Ubicación actualizada con precisión de ${precision.toStringAsFixed(1)}m')),
      );
    } catch (e) {
      setState(() {
        _isUpdatingLocation = false;
    ...
    if (latitudPosteSeleccionado != null) {
      markers.add(
        Marker(
          ...
          position: LatLng(
              latitudPosteSeleccionado!, longitudPosteSeleccionado!),
          infoWindow: InfoWindow(
  ...
  void _onPosteMoved(LatLng newPosition) {
    ...
    double distancia = _calcularDistancia(latitudUsuario!, longitudUsuario!,
    ...
    if (distancia <= 35) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'Poste reposicionado (${distancia.toStringAsFixed(1)}m del usuario)')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Muy lejos del usuario (${distancia.toStringAsFixed(1)}m). Máximo: 35m'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Los servicios de ubicación están deshabilitados. Por favor, activa el GPS en la configuración de tu dispositivo.'),
          actions: [
            TextButton(
              onPressed: () {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Sin acceso a la ubicación, no se puede registrar la posición del poste. ¿Desea intentar nuevamente?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text('Cancelar'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Los permisos de ubicación han sido denegados permanentemente. Debe habilitarlos manualmente en la configuración del dispositivo.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text('Cancelar'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'No se pudo obtener la ubicación. Verifica que el GPS esté activado y que tengas señal.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  void _onSiguientePressed() {
    ...
    // Navegar al paso 5 (Fotos) con TODOS los datos incluyendo GPS
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PosteTelematicoFotosScreen(
