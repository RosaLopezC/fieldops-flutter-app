...
import 'predio_fotos_screen.dart'; // <-- Import agregado aquí

class PredioGpsScreen extends StatefulWidget {
  final String distrito;
  final String zona;
  final String sector;
  final Map<String, dynamic> predioData;

  const PredioGpsScreen({
    Key? key,
  ...
  @override
  _PredioGpsScreenState createState() => _PredioGpsScreenState();
}

class _PredioGpsScreenState extends State<PredioGpsScreen> {
  ...
  // Posición seleccionada para el predio
  double? latitudPredioSeleccionado;
  double? longitudPredioSeleccionado;
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Inicialmente el predio está en la posición del sector
        if (latitudPredioSeleccionado == null) {
          latitudPredioSeleccionado = latitudSector;
          longitudPredioSeleccionado = longitudSector;
        }
      });
  ...
  void _updateMapMarkers() {
    ...
    if (latitudPredioSeleccionado != null) {
      markers.add(
        Marker(
          markerId: MarkerId('predio'),
          position: LatLng(latitudPredioSeleccionado!, longitudPredioSeleccionado!),
          infoWindow: InfoWindow(
            title: 'Posición del predio',
            snippet: 'Arrastra para mover',
          ),
          draggable: true,
          onDragEnd: (LatLng newPosition) {
            _onPredioMoved(newPosition);
          },
  ...
  void _onPredioMoved(LatLng newPosition) {
    ...
    if (distancia <= 35) {
      setState(() {
        latitudPredioSeleccionado = newPosition.latitude;
        longitudPredioSeleccionado = newPosition.longitude;
      });
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Predio reposicionado (${distancia.toStringAsFixed(1)}m del usuario)')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Sin acceso a la ubicación, no se puede registrar la posición del predio. ¿Desea intentar nuevamente?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text('Cancelar'),
  ...
  void _onSiguientePressed() {
    ...
    // Añadir datos de GPS al mapa de datos del predio
    final Map<String, dynamic> updatedPredioData = {
      ...widget.predioData,
      'latitudUsuario': latitudUsuario!,
      'longitudUsuario': longitudUsuario!,
      'latitudPredio': latitudPredioSeleccionado!,
      'longitudPredio': longitudPredioSeleccionado!,
      'precision': precision,
    ...
    Navigator.push(
      context, 
      MaterialPageRoute(
        builder: (context) => PredioFotosScreen(
          distrito: widget.distrito,
          zona: widget.zona,
          sector: widget.sector,
          predioData: updatedPredioData,
          latitudUsuario: latitudUsuario!,
          longitudUsuario: longitudUsuario!,
          precision: precision,
          fechaActualizacion: fechaActualizacion,
  ...
  Widget build(BuildContext context) {
    // No necesitamos usar el tipo específico de predio en el resumen
    // String tipoPredioText = widget.predioData['tipoPredio'] ?? 'Predio';
                        ...
                        Expanded(
                          child: _buildCoordinateField('Latitud', 
                            _mostrandoUsuario 
                              ...
                              : latitudPredioSeleccionado!.toStringAsFixed(7)
                          ),
                        ),
                        SizedBox(width: 15),
                        Expanded(
                          child: _buildCoordinateField('Longitud', 
                            _mostrandoUsuario 
                              ...
                              : longitudPredioSeleccionado!.toStringAsFixed(7)
                          ),
                        ),
                      ],
                    ),
